...
def validate_checkbt_param(param: int | str | Bloodtype):
    if type(param) not in [int, str, Bloodtype]:
        raise TypeError()

    if isinstance(param, int):
        if param not in [member.value for member in Bloodtype]:
            raise ValueError

    if isinstance(param, str):
        if param not in blood_type_text.keys():
            raise ValueError


def param_to_int(param: int | str | Bloodtype):
    if isinstance(param, int):
        return param
    elif isinstance(param, str):
        return blood_type_text[param].value
    elif isinstance(param, Bloodtype):
        return param.value
...
def check_bt(donor, recipient):
    ...
    """
    for param in [donor, recipient]:
        validate_checkbt_param(param)

    return all(
        [
            i > -1
            for i in _particular_antigen_comp(
                param_to_int(donor), param_to_int(recipient)
            )
        ]
    )
